...
export default function Sidebar() {
  ...
  return (
    <aside className="flex w-60 shrink-0 flex-col border-r border-sky-100 bg-white/60 p-4 shadow-[10px_0_35px_rgba(14,165,233,0.04)] backdrop-blur-xl">
          ...
          <NavLink
            ...
            className={({ isActive }) =>
              `rounded-xl px-3 py-2 text-sm transition-colors ${
                isActive
                  ? 'border border-sky-200 bg-white/80 font-medium text-sky-700 shadow-[0_8px_24px_rgba(14,165,233,0.08)]'
      ...
      <div className="mt-auto border-t border-sky-100 pt-3">
        <button
          onClick={() => logout()}
          className="w-full rounded-xl px-3 py-2 text-left text-sm text-zinc-500 transition-colors hover:bg-white/60 hover:text-zinc-800"
        >
          ออกจากระบบ
        </button>
      </div>
